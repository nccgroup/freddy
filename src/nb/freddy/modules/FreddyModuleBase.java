...
public abstract class FreddyModuleBase {
	...
	private static final byte[] ASCHEX_LOOKUP = {0x30, 0x31, 0x32, 0x33, 034, 0x35, 0x36, 0x37, 0x38, 0x39, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66};
	...
	protected String encodeStringToAsciiHex(String base) {
		...
		for(int i = 0; i < strBytes.length; ++i) {
			sb.append(ASCHEX_LOOKUP_TEXT[((strBytes[i]) >> 4) & 0xf]);
			sb.append(ASCHEX_LOOKUP_TEXT[(strBytes[i]) & 0xf]);
		}
		return sb.toString();
	}
	...
	private IScanIssue createPassiveScanIssue(IHttpRequestResponse baseReqRes, List<int[]> reqMarkers, List<int[]> resMarkers) {
		...
		if(reqMarkers.size() > 0 && resMarkers.size() > 0) { issueDescription += "The HTTP request and response "; }
		else if(reqMarkers.size() > 0) { issueDescription += "The HTTP request "; }
		else if(resMarkers.size() > 0) { issueDescription += "The HTTP response "; }
		...
		if(reqMarkers.size() > 0 && resMarkers.size() > 0) { confidence = ConfidenceRating.FIRM; severity = _severity; }
		else { confidence = ConfidenceRating.TENTATIVE; severity = _severity; }
